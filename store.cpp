...
void Store::borrowMovie(char movieGenre, ifstream movie//
// Created by stlp on 11/28/20.
//
#include "store.h"
...
void Store::addMovieInventory(ifstream &infile)
{
    ...
    for(;;)
    {
        ...
        else if (genre == drama)
        {
            string stock;
            infile >> stock;
                                                // - 2 to delete the comma & needs to be 10
            stock = stock.substr(0,stock.length() - 2); //////// make sure this works.
            string director;
            infile >> director;
            string title;
...
void Store::doCommands(ifstream &infile)
{
    for(;;)
    {
        ...
        else if (command == 'B')
        {
            ...
            infile >> movieGenre;
            if (movieGenre != 'D' && movieGenre != 'C' && movieGenre != 'F') {} ////INVALID GENRE ERROR
            borrowMovie(Customer borrower, movieGenre, infile);
